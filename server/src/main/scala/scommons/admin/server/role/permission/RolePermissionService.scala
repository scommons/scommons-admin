...
class RolePermissionService(roleDao: RoleDao,
  ...
  def listRolePermissions(systemId: Int, roles: Set[Role]): Future[List[(Permission, Boolean)]] = {
    permissionDao.list(systemId, roles.map(_.id))
  }
  ...
  def addRolePermissions(role: Role, permissionIds: Set[Int]): Future[Option[List[(Permission, Boolean)]]] = {
    ctx.transaction { implicit ec: ExecutionContext =>
      for {
        currPermissions <- permissionDao.list(role.systemId, Set(role.id))
        ...
        res <-
          if (updated) permissionDao.list(role.systemId, Set(role.id)).map(Some(_))
          else {
