 ...
 * 
 * Copied from: https://github.com/Ticketfly/play-liquibase/blob/master/play-liquibase/src/main/scala/com/ticketfly/play/liquibase/PlayLiquibaseModule.scala
 */
class LiquibaseModule extends Module {
...
class PlayLiquibase @Inject()(environment: Environment, config: Configuration, injector: Injector) {
  ...
  protected def liquibase(): Option[Liquibase] = {
     ...
     */
    def getRequiredParam(p: Param, defValue: Option[String])
                        (implicit cfg: Option[Configuration]): String = {
